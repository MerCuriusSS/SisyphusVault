...
🎯 XSS 过滤器的三层防护机制
  ...
  RuoYi-Vue-Plus 使用了双重防护策略：                                                                                                                                                                              
  ┌─────────────────────────────────────────────────────────┐                                                                                                                                                        │ 第一层：XssFilter (Filter 层)                             │
  │ - 拦截所有请求                                           │
  │ - 通过 XssHttpServletRequestWrapper 包装请求             │
  └─────────────────────────────────────────────────────────┘
                          ↓                                                                                                                                                                                       
  ┌─────────────────────────────────────────────────────────┐
  │ 第二层：@Xss 注解 + XssValidator (参数校验层)             │
  │ - 在 Controller 参数上使用                               │
  │ - 对字段进行精确校验                                     │
  └─────────────────────────────────────────────────────────┘

  ---
  🔬 核心原理：HttpServletRequestWrapper 模式

  1️⃣ Wrapper 模式的本质                                                                                                                                                                                          

  XssHttpServletRequestWrapper 继承了 HttpServletRequestWrapper，通过装饰器模式重写关键方法：

  // XssHttpServletRequestWrapper.java:27
  public XssHttpServletRequestWrapper(HttpServletRequest request) {
      super(request);  // 持有原始 request 的引用
  }

  核心思想：
  原始请求
      │
      ├─ getParameter("name")     → 被拦截 → 清理 XSS                                                                                                                                                            
      ├─ getParameterValues()     → 被拦截 → 清理 XSS                                                                                                                                                            
      ├─ getParameterMap()        → 被拦截 → 清理 XSS                                                                                                                                                            
      └─ getInputStream()         → 被拦截 → 清理 JSON 中的 XSS                                                                                                                                                  

  ---
  2️⃣ 三种数据来源的清理方式                                                                                                                                                                                      

  场景 A：URL 参数 / Form 表单参数

  // XssHttpServletRequestWrapper.java:36-42
  @Override
  public String getParameter(String name) {
      String value = super.getParameter(name);  // 获取原始值
      if (value == null) {
          return null;
      }
      return HtmlUtil.cleanHtmlTag(value).trim();  // 清理 HTML 标签
  }
  ...
  // 清理后
  String output = HtmlUtil.cleanHtmlTag(input);
  // output = "Hello  World"  (script 标签被删除)

  ---
  场景 B：多值参数（如 checkbox）

  // XssHttpServletRequestWrapper.java:69-81
  @Override
  ...
  ---
  场景 C：JSON 请求体 (@RequestBody)

  // XssHttpServletRequestWrapper.java:84-125
  @Override
  public ServletInputStream getInputStream() throws IOException {
      // 1. 判断是否为 JSON 请求
      if (!isJsonRequest()) {
          return super.getInputStream();  // 非 JSON 直接放行
      }
      ...
      // 3. 清理 JSON 中的 HTML 标签
      json = HtmlUtil.cleanHtmlTag(json).trim();

      // 4. 重新包装成 ServletInputStream 返回
      byte[] jsonBytes = json.getBytes(StandardCharsets.UTF_8);
      final ByteArrayInputStream bis = IoUtil.toStream(jsonBytes);
      return new ServletInputStream() { /* ... */ };
  }

  ⚠️ 注意：这种方式会破坏 JSON 结构！                                                                                                                                                                            

  // 原始 JSON（合法）
  {"content": "<p>Hello</p>"}

  // 清理后（可能非法）
  {"content": "Hello"}  // 如果 "Hello" 后面有逗号，会变成非法 JSON

  ---
  🛡️ HtmlUtil.cleanHtmlTag() 的实现原理                                                                                                                                                                           

  这是 Hutool 工具库提供的方法，底层使用正则表达式：
  ...
  public static String cleanHtmlTag(String html) {
      ...
      // RE_HTML_MARK = "<[^>]+>"  匹配所有 HTML 标签
      return html.replaceAll(RE_HTML_MARK, "");
  }
  ...
  匹配示例：
  <script>alert(1)</script>  → 完全匹配                                                                                                                                                                           
  <div>text</div>            → 完全匹配                                                                                                                                                                           
  <img src=x onerror=...>    → 完全匹配                                                                                                                                                                           

  ---
  📋 XssFilter 的完整工作流程

  // XssFilter.java:33-43
  @Override
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) {
      ...
      if (handleExcludeURL(req, resp)) {
          chain.doFilter(request, response);  // 不包装，直接放行
          return;
      }
      ...
      // 3. 传递包装后的请求给下一个 Filter/Controller
      chain.doFilter(xssRequest, response);
  }

  排除规则（XssFilter.java:45-53）：

  private boolean handleExcludeURL(HttpServletRequest request, HttpServletResponse response) {
      String url = request.getServletPath();
      String method = request.getMethod();

      // GET、DELETE 请求不过滤（通常不会有请求体）
  ...
  xss:
    enabled: true
    excludeUrls:
      - /system/notice/*  # 通知模块允许富文本
      - /tool/gen/edit    # 代码生成允许 HTML

  ---
  🔍 第二层防护：@Xss 注解

  当 Filter 层不足以满足需求时，可以使用注解级别的精确校验：

  // XssValidator.java:14-19
  ...
  public class UserController {
      ...
      @PostMapping("/user")
      public Result save(@RequestBody @Xss UserBo user) {
          // 如果 user.name 包含 <script>，会直接抛出异常
          // 不会进入业务逻辑
  ...
  │     问题      │                        说明                        │                  解决方案                  │
  ├───────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────┤
  │ JSON 结构破坏 │ cleanHtmlTag() 会删除所有标签，可能破坏合法的 JSON │ 对于富文本场景，使用排除路径或更精细的清理 │
  ├───────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────┤
  │ 绕过方式      │ 使用 Base64 编码、Unicode 转义等                   │ 结合其他安全措施（CSP、输入验证）          │
  ├───────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────┤
  │ 性能影响      │ 每次请求都要解析和清理 JSON                        │ 合理配置排除路径                           │
  ├───────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────┤
  │ 误杀          │ 合法的富文本内容也被清理                           │ 使用白名单模式（如 Jsoup）                 │
  └───────────────┴────────────────────────────────────────────────────┴────────────────────────────────────────────┘
  ---
  🎓 总结：XSS 过滤器的防护策略
  ...
  ┌─────────────────────────────────────────────────────────────┐
  │  XssFilter.doFilter()                                        │
  │  ├─ 检查是否在排除列表（GET/DELETE + 配置的路径）            │
  │  ├─ 如果排除 → 直接放行                                      │                                                                                                                                                
  │  └─ 如果不排除 → 包装为 XssHttpServletRequestWrapper         │                                                                                                                                                
  └─────────────────────────────────────────────────────────────┘
                              ↓                                                                                                                                                                                   
  ┌─────────────────────────────────────────────────────────────┐
  │  XssHttpServletRequestWrapper                                │
  │  ├─ getParameter()     → HtmlUtil.cleanHtmlTag()           │                                                                                                                                                  
  │  ├─ getParameterValues() → 逐个清理                        │                                                                                                                                                  
  │  ├─ getParameterMap()    → 遍历清理所有参数                │                                                                                                                                                  
  │  └─ getInputStream()    → 读取 JSON → 清理 → 重写流        │                                                                                                                                                
  └─────────────────────────────────────────────────────────────┘
                              ↓                                                                                                                                                                                   
  ┌─────────────────────────────────────────────────────────────┐
  │  Controller 层可选防护                                       │
  │  └─ @Xss 注解 + XssValidator → 抛出异常或拒绝请求           │                                                                                                                                                 
  └─────────────────────────────────────────────────────────────┘
                              ↓                                                                                                                                                                                   
  ┌─────────────────────────────────────────────────────────────┐
  │  业务逻辑（收到的已经是清理后的数据）                         │
  ...
  关键点：
  1. Wrapper 模式是核心，透明地拦截所有参数读取
  2. 双重防护：Filter 层全局拦截 + 注解层精确校验
  3. 配置灵活性：通过 xss.excludeUrls 排除富文本场景
  4. 基于正则：HtmlUtil.cleanHtmlTag() 使用正则删除所有 HTML 标签

  如果你需要更精细的 XSS 防护（如保留部分标签），可以考虑使用 Jsoup 的白名单模式替代 Hutool 的正则方式。需要我给你展示如何实现吗？