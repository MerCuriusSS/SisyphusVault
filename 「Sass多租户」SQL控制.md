...
| ------ | ------------------------- | ------------ | ----------------------------------------------------------------------------------- |
| 租户切换   | 依赖登录用户，切换用户会登录失效          | 不依赖登录态       | 1）Redis专门保留动态租户ID<br>2）Redis全局存Token，不跟租户绑定<br>3）查数据时优先用这个 ID，不用重新登录                |
| 业务数据缓存 | 需要手动添加「租户ID」前缀作租户隔离，以防串数据 | 自动添加「租户ID」前缀 | 1）框架自动给 Key 加 “租户 ID 前缀”，不同租户的 Key 不一样；<br>2）如果是全局数据（比如 global:config），就不加前缀，所有人都能查 |
|        |                           |              |                                                                                     |
