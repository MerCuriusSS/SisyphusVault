...
### 🟣 核心定义：服务器干活的核心，主要用于执行「计算」和「线程/资源管理」指令；等待IO操作时不工作。
### 🟣核心价值：衡量“CPU 这个零件累不累”的指标
...
### 🟣结构组成：
- us（用户态）
	- 定义：描述应用**程序代码**消耗的CPU情况（业务逻辑、循环、JSON解析、**GC**等）
- sy（系统）
	- 定义：描述操作系统管理**线程/资源时消耗**的CPU情况（线程切换、锁竞争、CAS自旋、销毁/创建）
