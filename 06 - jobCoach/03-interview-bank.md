...
---
您好，我有 5 年 Java 后端开发经验，主要负责 ToB 核心业务系统、数据平台和高吞吐数据处理链路建设。熟悉的技术栈主要包括 Spring Boot、Spring Cloud、MyBatis、MySQL、Redis、kafka消息中间件、Docker 以及监控排障体系。（放慢语调、重读来回忆后续衔接...）

最近一段经历中，我主要负责「多源数据同步 NAS 落盘系统」、「数据库元数据管理平台」这类偏数据处理和高吞吐稳定性的项目。比如在 NAS 落盘系统中，我负责从 0 到 1 的架构设计和核心实现，通过异步执行引擎、At-Least-Once 语义、批量刷盘和消费位点恢复，支撑日均 7.5TB 数据稳定落盘。我的优势是后端工程落地、性能优化和系统稳定性治理。（衔接...）

最近我个人也开发并落地部署了名为“拓码”的 AI 零代码应用生成平台，尝试把 Java 后端能力和大模型应用结合起来，其主要功能是通过自然语言生成应用，支持可视化编辑、AI对话式修改以及一键部署分享。现在希望找 Java 后端、AI 应用后端方向的岗位。
...
在项目的整个开发过程中，遇到了几个棘手的问题：
第一是针对kafka类型的单线程"拉取-序列化-写盘"全串行架构存在性能瓶颈，消费拉取与写盘阻塞会互相拖累，在业务高峰时段消息积压明显，下游消费延迟由秒级恶化至分钟级。
（消费拉取和写盘的全串行链路性能低、业务高峰期积压明显、下游消费延迟恶化）
第二是kafka采用自动提交策略，消费不可靠，断电或异常恢复出现数据丢失。
第三是任务启停和分布式调度依赖数据库轮询，会造成额外压力和响应延迟。


为此我设计了以下针对方案。
针对写入性能，我重构了异步执行引擎，采用多 Consumer 并行拉取序列化、10 万级阻塞队列背压和单 Writer 批量写入的架构，以及小时窗口滚动文件生成方式。既实现了消费拉取、写盘阻塞的异步解耦，又减少系统调用次数。
经本地实测，改造前后每条1kb数据的单核写路径峰值分别为 37.3万TPS、56.3万TPS，提升50%，生产实测改造前后单核写路径峰值分别为 26万TPS（300MB/s）、33.8万TPS（425MB/s），提升30%，最终支撑日均 7.5TB 的数据落盘，实现全天零积压，下游消费延迟稳定保持在秒级。

针对可靠性，我设计了跨线程 ackQueue 回传机制，采用“先落盘、后提交”的 At-Least-Once 语义，只有在批数据 flush排空页缓存后才提交消费位点。断电或异常恢复时，也可以通过文件偏移量解析重置消费位点，避免数据丢失和降低重复成本。
...
- 为什么一天的数据只有7.5TB：
  ...
  2）再进一步讲，一天内的高峰流量大概有4小时，低峰流量持续20小时，结合压测出的模型实际吞吐量大小，以及高峰时段堆积最大堆积数据量500g，可计算出高峰的吞吐量可达380MB/s，低峰时段大概在30MB/s。改造后架构数据吞吐量可达425Mb/s，可完美覆盖峰值场景
  

- TPS 结果如何计算出来？
  通过绕过真实kafka，纯内存生成消息、每条消息固定为1kb大小，分别对 单线程架构 和 多线程架构 进行为时60s的压测得出的。
  期间通过原子计数器和ThreadMXBean 热点探针分别采样消息总数和CPU占用时间得到TPS和单核CPU使用占比，验证CPU和带宽的瓶颈所在。

- 如何计算CPU使用占比？
  ThreadMXBean分别采集线程执行一秒前后CPU累加值作差值运算，结果与1s的比值即为CPU使用占比。

- 为什么本地1.5倍，生产只有1.3倍？
  本地环境是本地写入，生产环境是NAS盘写入，存在网络IO损耗。
...
- 消费位点重置实现原理？
  【封装的消息体携带offset】启动时扫描当天所有文件尾部1MB【randomAccessFile.seek()】方法，得到各分区最新offset，待消费者分配完分区后，手动跳转到文件所示offset继续消费。
...
- pub/sub的发布订阅实现原理？
  节点订阅同一个频道，在项目启动时触发，点击“取消”时，对应请求会“广播”到各个节点，工作节点收到后直接匹配本地内存中的异步线程进行中断，做到了跨节点调度的0延迟和数据库0压力。
  
  可靠性保证，基于pub/sub，添加一个轮询数据库的定时任务，但频率更低（3分钟左右），是兜底保证。
...
为了解决采集稳定性问题，我采用只读从库路由和 EMA 限流策略，降低对业务库的影响

为了解决比对性能问题，我设计了多级 MD5 摘要联动哈希比对方案，将表名称、字段、约束等不同层级的结构信息拼接并生成摘要。版本比对前先判断上层摘要是否变化，99% 无变更对象可以直接剪枝，避免深层字段逐项比较。

---
可能追问点：
- 异构数据库系统表结构不同，如何抽象定义？
  定义了元数据接口，
  统一定义了元数据接口，按表结构、字段、约束、索引几个维度查询划分方法，结合【策略模式】完成异构数据库表的具体实现。

- 异构数据库类型
  MySQL、Oracle、SQLServer、PGSQL
...
- 版本比对的表设计思路：
  ...
  a.主键
  b.关联表id
  c.版本信息（当前版本号、变更类型、具体字段变更前后值）
  d.审计字段

  
- 限流器的设计？
  采用guava实现限流器，每个数据源独立连接池&独立限流器。每次执行SQL时才触发限流判断。
  按照请求错误率、连接池使用率、基于EMA算法的平均响应时间几个指标作为限流标准进行设计的，规则是当连接池使用率>80%、平均响应时间>500ms、错误率>5% 三者满足其一就触发限流，当三者都不满足时，取消限流

- 多数据源实例创建
  1.手动创建，非spring容器管理
  2.Caffeine 本地缓存——懒加载、空闲过期淘汰、
	  - 懒加载：用时才创建，符合动态数据源语义
	  - 空闲过期淘汰：自动销毁实例、避免长时驻留导致内存泄漏
  
- 什么是EMA？怎么用？
  EMA指的是指数移动平均，作用是对波动数据和大样本数据分配不同权重以降低毛刺影响。在项目中主要用于“平均响应时间”的计算，按照 历史平均、最后一次响应时间的 8:2 权重进行分配，以避免极端响应时间直接触发限流；
...
2.@Transactional的核心属性：事务传播特性（propagation）、事务隔离级别（isolation）、回滚触发异常（rollbackFor）
	1）事务传播特性：REQUIRED(默认、**包含**关系下最多只有一个当前事务)、REQUIRES_NEW（**包含**关系下每次新建事务，提交方式是先内后外）
	2）事务隔离级别：
...
3. 缓存雪崩：
	...
	2. **解决方案**：
		1.  TTL 加随机值，避免同一时间过期。
		2. Redis 高可用：哨兵 / 集群 [Redis 哨兵与集群](../03%20-%20Areas/Redis%20哨兵与集群.md)
...
- CMS GC
	...
	- 缺陷：
		- 只回收老年代，采用【标记 - 清除】算法，产生内存碎片（内存碎片）
...
- G1 GC
	...
	- 性质：整堆GC
	- 优化：
		- 分区：按逻辑分代（贴标签区分，不再物理划分）切割成若干个不同类型的区域；每类区域分配不同内存，提升回收效率；
		- 无碎片：微观各分代统一采用 标记-复制算法对数据进行迁移整理。
		- 停顿可控：局部（分区）增量回收，多回收新生代，少回收老年代，更快响应
	- GC过程：
		1. 初始标记：(stw)
		2. 并发标记：(不卡业务)
		3. 重新标记:（stw）
...
>重点区分：
>GC 日志 ≠ Heap Dump
>**GC日志**：监控录像，记录一段时间 JVM GC 发生的全过程；**随程序运行持续流式生成**
>**Heap Dump**：堆内存各对象细节快照；**触发式的瞬时快照**
...
- MAT 分析路径：
	...
	7. 判断是谁持有引用、对象内容是什么
	8. 结合业务内容、引用变量名称进行代码修复

```
Leak Suspect：一个报告总述，大致描述疑似泄露的点在哪里
Dominator Tree：对象实例层面的内存占比展示（按retained heap 字段排序）
├─ 展开可看到对象内容
├─ Path-to-root（exclude weak reference）可看到引用链（自底向上）以及引用名称（实例变量、静态变量可看到字段名、局部变量不可见）
...
- 改原有索引：调整索引顺序——利用高频、可选择性强设计原则
- 改SQL：减少查询字段、大分页改为游标分页、拆分复杂join等
- 新增索引：串联整个查询路径（字段->where->排序、分组）
...
2.volatile规则：
volatile变量的写 happens-before 后续对这个变量的读 （对volatile的写对所有线程可见）
...
- 用于原子更新的乐观并发控制机制。会在更新前比较「内存值」与「期望旧值」是否相等，相等则更新「新值」，否则更新失败。用于实现对变量的原子性修改
```java
...
自旋消耗CPU：通过一个`while(true)`死循环完成对变量的原子性更新，失败时不断重试。
...
3）CAS+ park/unpark：
- 通过 CAS 划分失败与成功线程
- 成功线程修改 `state`状态，失败线程进入队列，执行`LockSupport.park()`；等锁释放时，再通过`LockSupport.unpark(thread)`唤起后继线程。
...
- 线程池管 java 线程，属于「计算」资源，连接池管 JDBC Connection(数据库TCP请求)，属于「IO」资源
- 连接池中的连接资源 消耗与归还是以「事务」为单元，而不是单纯的以DAO接口个数界定。
...
**注意：不要拿 heap dump（jmap）排查线程池打满**。heap dump 看对象，看不到线程运行栈；线程栈只能 jstack**

- 运行问题看**线程栈**——线程卡顿、线程池打满、死锁、阻塞等待
- 内存问题看**堆快照**——有哪些对象、对象大小、引用链
