...
4.异步执行引擎、At-Least-Once 机制、批量刷盘和消费位点恢复、1.5T
5.落地、优化、治理
6.后端能力、大模型应用
7.后端、应用后端
...
最近一段经历中，我主要负责「多源数据同步与 NAS 落盘系统」、「数据库元数据管理平台」这类偏数据处理和高吞吐稳定性的项目。比如在数据落盘系统中，我负责从 0 到 1 的架构设计和核心实现，通过异步执行引擎、At-Least-Once 机制、批量刷盘和消费位点恢复，支撑日均 1.5TB 数据稳定落盘。我的优势是后端工程落地、性能优化和系统稳定性治理。（衔接...）

最近也在做 拓码 这种 AI 零代码应用生成平台，尝试把 Java 后端能力和大模型应用结合起来。现在希望找 Java 后端、AI 应用后端方向的岗位。
...
专业术语关键词：
- 控制面、数据面、数据源接入、任务配置、启停调度、数据拉取写入
- 单线程、消息积压、NAS盘、系统调用、写入抖动
- 自动提交、不可靠、断电或异常恢复、数据丢失
- 启停调度、依赖、额外压力、响应延迟
- 写入性能、异步执行引擎、多consumer、10万级阻塞队列、单writer、批量Flush、无锁化滚动文件切割、系统调用、2.4万TPS、1.5T
- 可靠性、跨线程ackQueue回传机制、 先落盘后提交、消费位点、文件偏移量解析、数据丢失、重复消费
- 任务调度、定期扫表、Redis 发布订阅模型、近实时响应、轮询压力
...
可能追问点：
- TPS结果如何得出？
  非精确答案，只是合理推导。本地造数、队列满载且数据按每条1kb的条件下，TPS可达25万TPS/s，结合瓶颈位在单线程写入磁盘的结论，得出本地磁盘吞吐量约为250MB/S，生产环境中的NAS盘，常规吞吐量大概为50MB/s，再算上OS其他操作损耗、网络延迟等打5折，也有25MB/s，预估2.4万TPS/s，绰绰有余甚至偏保守。

- 为什么flush窗口要规定是1s？
  1）1s频率让NAS调用次数维持在8.6万次，在NAS盘承受范围内
  2）处于边际拐点，太低CPU开销暴增、太高数据丢失量不可接受

- 跨线程ackQueue回传机制实现原理？
  先按消费者人均一个ackQueue->拉取的消息携带引用->写入消息时按ackqueue进行分组，合并多个分区 offset最值后放回对应ackQueue->消费者ACK
...
这个项目是数据库元数据管理系统，主要目标是解决数据资产不透明、库表变更不可追踪的问题。我的工作是设计、采集链路、版本比对能力
...
- EMA 动态背压是如何实现的？什么是EMA？怎么用？
  EMA指的是加权滑动平均算法，用于对波动数据加权过滤毛刺。
  动态背压的设计我是采用guava限流器，并按照请求错误率、连接池使用率、使用EMA算法的平均响应时间几个指标作为限流标准进行设计的，当连接池使用率>80%、平均响应时间>200ms、错误率>5% 三者满足其一就触发限流。
...
[Spring 事务与 MySQL 事务 记忆骨架](../Spring%20事务与%20MySQL%20事务%20记忆骨架.md)
...
4.快照读与当前读
	...
	2）当前读：读取最新数据，可能加锁。幻读解决手段：行锁+间隙锁（防新数据进）
		...
		b.select..... lock in share mode （S锁，读改情况下容易出现）
...
[Redis缓存一致性记忆骨架](../Redis缓存一致性记忆骨架.md)
...
1. 局限：无法保证强一致性：所有方案都存在**毫秒级短暂窗口**（DB与Redis非原子性导致的间隙）
...
| MQ 异步删缓存           | ≤50ms                 | 更新完DB后发送MQ消息，消费者接收后执行删除redis缓存               | 200ms~2s      | 高，消息重试不丢失     |
...
| 中小型分布式、中等写并发、无 MQ、可接受 1s 阻塞          | 同步延迟双删            |
| 高并发读、多服务改同表、不能阻塞写接口、已有 MQ            | MQ 异步删缓存          |
| 多渠道改库（后台 / DBA 脚本）、金融 / 核心交易展示、零漏删要求 | Canal Binlog + MQ |
...
现象3完美解决：消息表记录消息完整发送状态（待发送、成功、失败），生产者依托数据库状态前程可见
...
[JVM&GC&OOM排查记忆骨架](../JVM&GC&OOM排查记忆骨架.md)
...
| 方法区/元空间 | 否    | 类元数据、方法元信息、常量池等   | OutOfMemory Error：Meta Space |
...
```
1.类加载检查（加载->验证->准备->解析->初始化）
2.分配内存（根据[成员变量、对象头、实例数据、对齐填充]确定内存大小，通过指针碰撞/空闲列表分配堆内存）
3.设置对象零值（属性赋默认值）
...
- CMS GC
	...
	- **并发失败**：
	```
	1.只回收老年代，采用【标记 - 清除】算法，不整理内存、会产生碎片（清除不干净）
	2.触发时机晚，达到70%才并发回收，预留空间小（触发时机晚）
	3.持续加压，内存碎片和晋升大对象持续挤占空间
	4.serial Old兜底，全堆GC，STW时间更长，性能退化，卡顿严重（serial old 兜底）
	```
- G1 GC
	- 特点：大堆管理、分区、无碎片、停顿可控。
	- 性质：整堆GC
	- 优化：
		- 分区：逻辑分代（贴标签区分，不再物理划分），多回收新生代，少回收老年代，提升回收效率；
...
[MySQL 索引与慢 SQL 调优记忆骨架](../MySQL%20索引与慢%20SQL%20调优记忆骨架.md)
...
[Java 并发知识体系记忆骨架](../Java%20并发知识体系记忆骨架.md)
...
- Happens-before：JMM 中可见性、有序性的规则
```
...
3.锁规则
对一把锁的unlock happens-before 后续对同一把锁的lock
...
`while`就是自旋：失败时不断重试；
低竞争——大部分线程一次成功，不用阻塞和唤醒，性能高
高竞争——大量线程争抢，只有一个成功，其他线程继续重试又大量失败，CPU被空转消耗，CPU升高。

原理：自旋性能由自旋整体次数决定，低竞争时并发线程少，总自旋次数少，消耗 CPU 少；高竞争时并发线程多，总自旋次数多，消耗 CPU 多。
...
```
自研锁底层逻辑繁杂，JDK 抽象 AQS 机制供各类并发工具复用，负责解决：
1.管理同步状态（锁是否占用）
...
Heap Configuration:
   MinHeapFreeRatio         = 40
   MaxHeapFreeRatio         = 70
   MaxHeapSize              = 891289600 (850.0MB)
   NewSize                  = 1363144 (1.2999954223632812MB)
   MaxNewSize               = 534773760 (510.0MB)
...
G1 Heap:
   regions  = 850
   capacity = 891289600 (850.0MB)
   used     = 52877056 (50.427490234375MB)
   free     = 838412544 (799.572509765625MB)
   5.932645909926471% used
G1 Young Generation:
Eden Space:
   regions  = 9
   ...
   10.112359550561798% used
Survivor Space:
   regions  = 1
   capacity = 2097152 (2.0MB)
   used     = 1247832 (1.1900253295898438MB)
   free     = 849320 (0.8099746704101562MB)
   59.50126647949219% used
G1 Old Generation:
   regions  = 41
   capacity = 55574528 (53.0MB)
   used     = 42192040 (40.237464904785156MB)
   free     = 13382488 (12.762535095214844MB)
   75.91974510336821% used
```
...
ss -lntp # 查看监听端口 和 进程
netstat -lntp # 查看监听端口 和 进程 l(listening) ；t(只筛选tcp协议)；p（端口号）
```
